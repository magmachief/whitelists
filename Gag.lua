...
local AutoBuyGear  -- For automatically buying gear
-- Table to store the selection state for each gear item.
local SelectedGears = {}

local function CreateWindow()
	local Window = ReGui:Window({
		Title = `{GameInfo.Name} | Jiwoo`,
        Theme = "GardenTheme",
...
-- Updated function: Buy all selected gear by iterating over multiple gear selections.
local function BuyAllSelectedGears()
	for gear, selected in pairs(SelectedGears) do
		if selected then
			-- Purchase each selected gear a set number of times (e.g., 3 times)
			for i = 1, 3 do
				BuyGear(gear)
				wait(0.2)
			end
...
local function StartServices()
	...
	--// Auto-Buy for Gear (if enabled, loop through each selected gear)
	MakeLoop(AutoBuyGear, function()
		for gear, selected in pairs(SelectedGears) do
			if selected then
				BuyGear(gear)
				wait(0.2)
...
--// Auto-Gear
local GearNode = Window:TreeNode({Title="Auto-Gear 🛠️"})
-- Instead of only listing gear with stock, list all gear by using GetGearStock with false
for gearName, _ in pairs(GetGearStock(false)) do
	GearNode:Checkbox({
		Value = false,
		Label = gearName,
		Callback = function(_, Value)
			SelectedGears[gearName] = Value
		end
	})
end
AutoBuyGear = GearNode:Checkbox({
	Value = false,
	Label = "Enabled Auto Buy Gear"
})
GearNode:Button({
	Text = "Buy Selected Gear Now",
