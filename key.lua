...
local function enablePCView(enabled)
	...
	local cam=Workspace.CurrentCamera
	cam.CameraType=Enum.CameraType.Custom
	-- Removed use of TouchCameraEnabled since it doesn't exist 
	if enabled then
...
AutomatedTab:AddTextbox({Name="Custom Anti‑Slippery Friction",Default=tostring(customAntiSlipperyFriction),Flag="CustomAntiSlipperyFrict",TextDisappear=false,Callback=function(value)
	...
	if num then customAntiSlipperyFrict=num; customAntiSlipperyFriction=num; print("Custom Anti-Slippery Friction updated to: "..num) end
end})
...
AutomatedTab:AddToggle({Name="PC View",Info="Enable PC-style overhead camera view; disables mobile thumbstick camera",Default=false,Callback=function(v)
	enablePCView(v)
end})
