...
local function faceNearestBombHolder()
	...
	if nearest and nearest.Character then
		...
		if bombHolderHRP then
			...
			-- Use CameraType Custom to let user control lateral movement while still orienting to bomb holder.
...
-----------------------------------------------------
-- PC View Functionality: Simulate PC camera with overhead view
-----------------------------------------------------
local function enablePCView(enabled)
	...
	cam.CameraType = Enum.CameraType.Custom
	if enabled then
		-- Set a default CameraOffset for an overhead look while allowing free look movement.
		LocalPlayer.CameraOffset = Vector3.new(0, 5, 0)
		cam.FieldOfView = 70
	else
...
AutomatedTab:AddToggle({Name="Face Bomb", Info="Automatically face the nearest bomb holder", Default=false, Callback=function(v)
	FaceBombEnabled = v
	if v then
...
AutomatedTab:AddToggle({Name="PC View", Info="Enable PC camera view with an overhead offset allowing free movement", Default=false, Callback=function(v)
	enablePCView(v)
end})
