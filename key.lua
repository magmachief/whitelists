...
function FrictionController.new()
	local self = setmetatable({},FrictionsController)
...
function FrictionController:calculateFriction(character)
	...
	if rayResult then
		...
		if floorPart:IsA("BasePart") and floorPart.Name=="Floor" then
			if floorPart.Friction<=0.2 then return math.clamp(customAntiSlipperyFriction*0.5,0.1,1.0) end
		end
	end
...
local function updateUIVisibility(hidden)
	...
	-- update ShiftLock UI transparency
	...
	-- update OrionLib UI transparency in PlayerGui and CoreGui
