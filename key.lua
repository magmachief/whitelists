...
local customBombFriction = 7  -- when holding bomb, if using FrictionController (unused in applyAntiSlippery)
local customAntiSlipperyFriction = customNormalFriction -- value when anti-slippery is ON
local fallbackFriction = 0.5 -- value when anti-slippery is OFF
...
local function applyAntiSlippery(enabled)
	if enabled then
			...
			while AntiSlipperyEnabledFlag do
				...
				for _, part in pairs(character:GetDescendants()) do
					if part:IsA("BasePart") then
						-- Apply the custom friction value (from menu)
		...
		for _, part in pairs(character:GetDescendants()) do
			if part:IsA("BasePart") then
				-- Restore to fallback friction value
...
-----------------------------------------------------
-- LEGACY FRICTION MODULE (for hitbox removal & fallback) [Unused if using applyAntiSlippery]
-----------------------------------------------------
...
-----------------------------------------------------
-- (Removed LegacyFrictionModule update loop in favor of applyAntiSlippery)
-----------------------------------------------------
...
AutomatedTab:AddToggle({Name="Anti-Slippery", Info="Uses custom friction value (normal: ~0.7, bomb state: your custom value)", Default=false, Callback=function(v)
	antiSlippery = v 
