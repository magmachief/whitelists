...
local function setUIVisualStealth(enabled)
    ...
    if ShiftLockScreenGui then
        rec(ShiftLockScreenGui)
    end
    for _, gui in ipairs(LocalPlayer:WaitForChild("PlayerGui"):GetChildren()) do
        if gui:IsA("ScreenGui") and gui.Name:match("Orion") then
            rec(gui)
        end
    end
...
local function createMobileToggle()
    ...
    button.Position = UDim2.new(1, -90, 1, -210)
...
LocalPlayer:WaitForChild("PlayerGui").ChildRemoved:Connect(
    function(child)
