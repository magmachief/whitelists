...
local allUIVisible = true  -- UI visible by default

-- Friction Controller Module
...
-- setUIVisualStealth toggles transparency but leaves UI interactive
local function setUIVisualStealth(stealth)
    local transVal = stealth and 1 or 0
    local function applyTransparency(gui)
        for _, obj in ipairs(gui:GetDescendants()) do
            if obj:IsA("GuiObject") then
                if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                    obj.TextTransparency = transVal
                elseif obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
                    obj.ImageTransparency = transVal
                end
                if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                    obj.BackgroundTransparency = transVal
                end
            end
        end
    end
    if mobileGui then applyTransparency(mobileGui) end
    if ShiftLockScreenGui then applyTransparency(ShiftLockScreenGui) end
    for _, gui in ipairs(LocalPlayer:WaitForChild("PlayerGui"):GetChildren()) do
        if gui.Name:match("Orion") and gui:IsA("ScreenGui") then
            applyTransparency(gui)
        end
    end
...
-- Mobile Auto Pass Button creation
