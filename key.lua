...
local customBombFriction=7  -- default set to 7 now so bomb holding gives high friction

-----------------------------------------------------
-- ENHANCED ANTI‑SLIPPERY MODULE
-----------------------------------------------------
...
function VisualModule.playPassVFX(target)
 ...
 emitter.Texture="rbxassetid://258128463"; emitter.Rate=50; emitter.Lifetime=NumberRange.new(0.3,0.5)
 emitter.Speed=NumberRange.new(2,5); emitter.VelocitySpread=30; emitter.Parent=hrp
...
-----------------------------------------------------
-- LEGACY FRICTION MODULE (for hitbox removal & fallback)
-----------------------------------------------------
...
local numRaycasts=5
local customAntiSlipperyFriction=customNormalFriction -- normal friction value used in legacy module (default 0.7)
local customHitboxSize=0.1

-----------------------------------------------------
-- UPDATE LEGACY FRICTION EVERY 0.5 SECONDS
-----------------------------------------------------
task.spawn(function()
 while true do
  if antiSlippery then LegacyFrictionModule.update() end
  wait(0.5)
 end
end)
...
AutomatedTab:AddToggle({Name="Anti-Slippery",Info="Uses custom friction value (normal: ~0.7, bomb state: your custom value)",Default=false,Callback=function(v) antiSlippery=v if not v then LegacyFrictionModule.restore() end end})
AutomatedTab:AddTextbox({Name="Custom Anti‑Slippery Friction",Default=tostring(customBombFriction),Flag="CustomAntiSlipperyFrict",TextDisappear=false,Callback=function(value)
 local num=tonumber(value) if num then customBombFriction=num; -- update both legacy and new module
  local fc=myFrictionController; if fc then fc.bombFriction=num end
  print("Custom Anti-Slippery Friction updated to: "..num) end end})
...
AutomatedTab:AddTextbox({Name="Custom Hitbox Size",Default=tostring(customHitboxSize),Flag="CustomHitboxSize",TextDisappear=false,Callback=function(value)
 local num=tonumber(value) if num then customHitboxSize=num print("Custom Hitbox Size updated to: "..num) if RemoveHitboxEnabled then applyRemoveHitbox(true) end end end})
...
local function createMobileToggle()
 ...
 button.Name="AutoPassMobileToggle"; button.Size=UDim2.new(0,50,0,50); button.Position=UDim2.new(1,-70,1,-110)
