...
local allUIHidden = false  -- default: UI visible
...
-- Function to set transparency (invisible but still interactive)
local function updateUIVisibility(hidden)
	...
	if mobileGui then
		for _, obj in ipairs(mobileGui:GetDescendants()) do
			if obj:IsA("GuiObject") then
				...
				elseif obj:IsA("Frame") or obj:IsA("ScrollingFrame") or obj:IsA("TextLabel") then
	...
	-- OrionLib UI update in PlayerGui and CoreGui
	local function setOrionTransparency(gui)
	...
	for _, child in ipairs(LocalPlayer:WaitForChild("PlayerGui"):GetChildren()) do
		if child.Name:match("Orion") and child:IsA("ScreenGui") then
			setOrionTransparency(child)
		end
	end
...
-- Mobile Auto Pass Button: Circle, visible by default
...
LocalPlayer:WaitForChild("PlayerGui").ChildRemoved:Connect(function(child)
	if child.Name == "MobileToggleGui" then
		...
		if not LocalPlayer.PlayerGui:FindFirstChild("MobileToggleGui") then
			...
			updateUIVisibility(allUIHidden)
		end
	end
end)
