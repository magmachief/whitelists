...
-----------------------------------------------------
-- SMART ANTI‑SLIPPERY FUNCTION (Old Style with enhancements)
-----------------------------------------------------
			...
			while AntiSlipperyEnabledFlag do
				...
				for _, part in pairs(character:GetDescendants()) do
					if part:IsA("BasePart") then
						if character:FindFirstChild(bombName) then
							-- Smart adjustment: if moving fast, increase friction moderately
...
function LoggingModule.logError(err, ctx)
	warn("[ERROR] Context: " .. tostring(ctx) .. " | Error: " .. tostring(err))
end
function LoggingModule.safeCall(func, ctx)
	local s, r = pcall(func)
	if not s then LoggingModule.logError(r, ctx) end
	return s, r
end
